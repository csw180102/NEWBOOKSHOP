		...
		<c:forEach var="vo" items="${bigVo}">
			<tr>
				<td align="center" style="width:3%" > 
					<input type="button" name="chk_stock" value="위시리스트" onclick="window.location='wishlist?book_seq=${vo.book_seq}'">
				</td>
				<td align="center"> <!-- 이미지 -->
			...
			<tr>
				<th>
					<c:if test="${pageNumStart > pageBlock}">
						<a href="booklist">[처음으로]</a>
						<a href="booklist?pageNum=${pageNumStart-pageBlock}">[◀]</a>
					</c:if>	
					...
					<c:if test="${pageTotalCnt > pageNumEnd}">
						<a href="booklist?pageNum=${pageNumStart + pageBlock}">[▶]</a>
						<a href="booklist?pageNum=${pageTotalCnt}">[마지막으로]</a>
					</c:if>
				</th>
			</tr>
		</table>
