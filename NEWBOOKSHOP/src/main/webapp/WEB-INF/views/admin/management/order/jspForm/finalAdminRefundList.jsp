...
<html>
<body>
<h3><a>---------finalAdminRefundList.jsp----------</a></h3>
<h2><center>최종 관리자 환불승인완료 목록 </center></h2>
			...
			<tr>
				<th>
					<c:if test="${pageNumStart > pageBlock}">
						<a href="finalrefundrequest">[처음으로]</a>
						<a href="finalrefundrequest?pageNum=${pageNumStart-pageBlock}">[◀]</a>
					</c:if>	
					...
					<c:forEach var="i" begin="${pageNumStart}" end="${pageNumEnd}">
			    		...
			    		<c:if test="${i != currentPage}">
			    			<a href="finalrefundrequest?pageNum=${i}">[${i}]</a>
			    		</c:if>
		    		</c:forEach>
					...
					<c:if test="${pageTotalCnt > pageNumEnd}">
						<a href="finalrefundrequest?pageNum=${pageNumStart + pageBlock}">[▶]</a>
						<a href="finalrefundrequest?pageNum=${pageTotalCnt}">[마지막으로]</a>
					</c:if>
				</th>
			</tr>
		</table>
